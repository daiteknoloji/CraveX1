...
const resourceMap = {
  ...
  room_messages: {
    map: (rm: RoomMessage) => ({
      ...rm,
      id: rm.event_id,
      body: rm.content?.body || "",
      msgtype: rm.content?.msgtype || rm.type,
      formatted_body: rm.content?.formatted_body || rm.content?.body || "",
    }),
    reference: (id: Identifier) => ({
      endpoint: `/_matrix/client/r0/rooms/${id}/messages`,
    }),
    data: "chunk",
    total: json => json.chunk?.length || 0,
  },
  pushers: {
    map: (p: Pusher) => ({
      ...p,
...
const dataProvider: SynapseDataProvider = {
  ...
  getManyReference: async (resource, params) => {
    ...
    // For room_messages, use different query parameters
    let query: any = {};
    if (resource === "room_messages") {
      // Messages endpoint uses 'dir' for direction (forward/backward)
      // Matrix Client-Server API uses 'limit' and 'dir'
      query = {
        limit: perPage,
        dir: "b", // backward - most recent first
    ...
    // For room_messages, add room_id to each message
    if (resource === "room_messages") {
      return {
        data: json[res.data].map((item: RoomMessage) => ({
          ...res.map(item),
          room_id: params.id,
        })),
        total: res.total(json, from, perPage),
